...
(async () => {
    try {
        ...
        const { inputFilePath, numRows, depth, numLeaves, originalRoot, sortedRoot } = await setup(csvFilePath);

        // Call the script that dynamically generates the circuits and creates the proofs.
        await generateZKProof(inputFilePath, datasetName, numRows, depth, numLeaves, originalRoot, sortedRoot );
