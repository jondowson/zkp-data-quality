...
async function generateZKProof(inputFilePath, datasetName, numRows, depth, originalRoot, sortedRoot) {
    try {
        console.log();
        console.log("--------------------------");
        ...
        const circuitFileMain = circuit_main(numRows);
        ...
        const circuitFileMerkle = circuit_merkleroot(numRows);
        ...
        const circuitFileUnique = circuit_unique(numRows);
        ...
        await exec_command(`circom ${filePath_main} --r1cs --wasm --sym --output ${generatedCircomDir} -l ${circomLibPath}`);
        ...
        //const witnessJsonData = fs.readFileSync(`${generatedSnarkjsDir}/witness.json`, 'utf8'); 
        // Parse the JSON data with the bigintReviver
        //const witnessData = JSON.parse(witnessJsonData, bigintReviver);
